...
mkdir -p $target_dir
cp $text_path $target_dir/lm.upper.lid.txt
...
cut -f1 -d' ' $target_dir/dict.txt | grep -Pv '\d\d\d\d\d+' >! $target_dir/words.txt
#cp $target_dir/dict.txt $target_dir/words.txt
python $FAIRSEQ_ROOT/examples/wav2vec/unsupervised/scripts/put_spaces_between_chars.py $target_dir/words.txt $target_dir/phones.txt
...
cp $target_dir/phones.txt $target_dir/lexicon.lst
echo waiting
sleep 4
...
#python $FAIRSEQ_ROOT/fairseq_cli/preprocess.py --dataset-impl mmap --trainpref $target_dir/lm.upper.lid.txt --only-source --destdir $target_dir/phones --thresholdsrc $min_phones --padding-factor 1 --dict-only
...
cp $target_dir/lexicon.lst $target_dir/lexicon_filtered.lst
...
python $FAIRSEQ_ROOT/examples/wav2vec/unsupervised/scripts/put_spaces_between_chars.py $target_dir/lm.upper.lid.txt $target_dir/phones/lm.phones.filtered.txt
cp $target_dir/phones/dict.txt $target_dir/phones/dict.phn.txt
#cp $target_dir/lm.upper.lid.txt $target_dir/phones/lm.phones.filtered.txt
